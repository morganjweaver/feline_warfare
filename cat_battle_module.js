...
var cat_battle = function() {
    var finished_round = false;
    while (finished_round === false) {
        var input = prompt("A laser cat emerges! Do you FIGHT, PAY by distracting it with valuable cat treats, or RUN?").toUpperCase();
        if(input === 'FIGHT' || input === 'PAY' || input === 'RUN') {
        switch(input) {
          case 'FIGHT':
          ...
          case 'RUN':
            ...
            break;
         default:
            console.log("That entry didn't make sense to me. Try again, this time picking FIGHT, PAY, or RUN!");
        } 
    }
}
}
